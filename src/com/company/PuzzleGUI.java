...
public class PuzzleGUI extends JPanel{
	...
	static ArrayList<String> matchedAC=new ArrayList<>();
	static ArrayList<String> answrAC=new ArrayList<>();

	static ArrayList<String> matchedDW=new ArrayList<>();
	static ArrayList<String> answrDW=new ArrayList<>();

	static int[] acrossLengths = new int [5];
	static int[] downLengths = new int [5];
	...
	public static void main(String[] args) throws IOException {
		...
		String[] dictionaryResults = new String[10];
		...
		try {
			...
			for(int a=0;a<10;a++){
				...
				if(a<5){
					dictionaryResults[a]=dictionary(myCluelist[a],acrossLengths[a]);
				}
				if(a>=5 && a<10){
					dictionaryResults[a]=dictionary(myCluelist[a],downLengths[a-5]);
				}
				System.out.println("dictionary result " +a+ " " +dictionaryResults[a]);
			}
			System.out.println();
		...
		for (int i = 0; i < word_starts.length; i++) {
			for (int j = 0; j < word_starts[0].length; j++) {
				if (word_starts[i][j] == ACROSS || word_starts[i][j] == BOTH) {
					...
					acrossLengths[acrossCount] = findWordLength(word_starts, i, j, true);
					System.out.println("across" + (acrossCount+1) + " = " + acrossLengths[acrossCount]);
					acrossCount++;
				}
				if (word_starts[i][j] == DOWN || word_starts[i][j] == BOTH) {
					...
					downLengths[downCount] = findWordLength(word_starts, i, j, false);
					System.out.println("down" + (downCount + 1) + " = " + downLengths[downCount]);
					downCount++;
				}
			}
		}
	...
	static int findWordLength(int[][] word_starts, int i, int j, boolean is_across) {
		int len;
	...
	public static String dictionary(String clue, int clueLength) throws IOException{

		//String clue="run for your wife";

		char[] charArray = clue.toCharArray();
		...
		for(int i=0;i<charArray.length;i++){
			if(Character.isWhitespace(charArray[i])){
				...
				newCharList.add(String.valueOf(charArray[i]));
			}
		}
		String s="";
		for(int x=0;x<newCharList.size();x++){
			s= s+(newCharList.get(x));
		}
		String url="http://www.dictionary.com/fun/crosswordsolver?query="+s+"&pattern=&l="+clueLength;
		//System.out.println(url);
		//System.out.println(newCharList);

		Document doc = Jsoup.connect(url).get();

		Elements words = doc.select("div[class]");

		int counter=0;
		int counter2=0;
		String[] myWordlist= new String[100];
		String x;
		String[] confidencelist= new String[100];


		for (Element word : words) {
			if((word.attr("class")).equals("matching-answer")){

				myWordlist[counter] = word.text();
				counter++;
			}
			if((word.attr("class")).equals("confidence")){
				confidencelist[counter] = word.text();
				counter2++;
			}
		}

		//System.out.println(myWordlist[1]);
		String con=confidencelist[1].substring(0,confidencelist[1].length()-1);
		int x2=Integer.valueOf(con);
	...
	public static void matchedClues()
	{
		StringMetric metric =
		...
		for (int i=0;i<5;i++)
		{
			for(int g=0;g<DATACLUE.size();g++)
			{
				...
				if(result>=0.7 && (DATAANSWER.get(g).length()==acrossLengths[i]))
				{matchedAC.add(myCluelist[i]);
					answrAC.add(DATAANSWER.get(g));}

				double sim1=similarity(DATACLUE.get(g),myCluelist[i+5]);
				float result1 = metric.compare(DATACLUE.get(g),myCluelist[i+5]);
					if(result1>=0.7 && (DATAANSWER.get(g).length()==downLengths[i]))
					{matchedDW.add(myCluelist[i+5]);
						answrDW.add(DATAANSWER.get(g));}
			}

		}
		if(matchedAC.size()==0)
			System.out.println(" NOTING ACROSS");
		if(matchedDW.size()==0)
			System.out.println(" NOTING DOWN");

		for(int y=0;y<matchedAC.size();y++)
			System.out.println("ACROSS MATCHED: "+ matchedAC.get(y)+" ANSWER: "+ answrAC.get(y));
		for(int y=0;y<matchedDW.size();y++)
			System.out.println("DOWN MATCHED: "+ matchedDW.get(y)+" ANSWER: "+ answrDW.get(y));
